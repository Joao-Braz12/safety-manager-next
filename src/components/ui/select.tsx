...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-12 w-full items-center justify-between bg-transparent",
  ...
  >
    ...
    <SelectPrimitive.Icon>
      <ChevronDown className="h-4 w-4 text-[var(--color-graphite-500)]" />
    </SelectPrimitive.Icon>
  </SelectPrimitive.Trigger>
));
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      position={position}
      className={cn(
        "z-50 min-w-[12rem] overflow-hidden border border-[var(--color-ink)] bg-[var(--color-bone)] shadow-[6px_6px_0_0_var(--color-ink)]",
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-pointer select-none items-center pl-3 pr-9 py-2",
      "text-sm text-[var(--color-ink)] outline-none",
      "focus:bg-[var(--color-ink)] focus:text-white",
